...
function windowScroll(){
	...
	$("#wrap").ready(function(){
		...
		var menu = $(this).find(".sampleBtn li")
		var frame = $(this).find("#contFrame")
		
		menu.find('span').bind('click',function(e){
			var par = $(this).parent()
			if(par.has('ul')) par.find('> ul').slideToggle('fast');
			return false;
		})
		...
		function resizeIframe(){
			...
			$(this).height($(this).contents().find('body')[0].scrollHeight + 150);
		});
	}
...
$(function(){
	var menu = $(this).find(".menu li");
	menu.find('span').bind('click',function(e){
		var par = $(this).parent();
		if (par.find('> ul').is(":hidden")){
			par.find('> ul').slideToggle('fast');
			par.find('b').text("-");
		} else {
			par.find('> ul').slideToggle('fast');
			par.find('b').text("+");
	...
	$(".js_toggle").bind("mousedown", function(){
		$(this).parent().find(".js_cont").slideToggle(300);
	});
